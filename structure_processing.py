...
# =====================================================
# 🔥 Alignment (SEPARATE CONCERN)
# =====================================================
def map_fasta_to_cif_parasail(fasta_seq, cif_seq, protein_id=None, logger=None):
    ...
    if logger:
        logger.warning(
            f"[ALIGN USED] {protein_id}: score={result.score}"
        )
    ...
    if coverage < 0.7:
        if logger:
            logger.error(
                f"[ALIGN REJECTED] {protein_id}: coverage={coverage:.2f}"
            )
        return None
    ...
    try:
        ...
        if is_alphafold:
            ...
            local_conf[has_structure] = (
                full_occ[has_structure] * sigmoid[has_structure]
            )
...
def build_graph_shards(
    ...
    print(f"Label to shard length: {len(label_to_shard)}| Total written: {total_written}")
