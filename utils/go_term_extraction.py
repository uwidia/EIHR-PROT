...
def build_go_annotations_list(
    ...
    *,
    ...
    label_to_go_terms = {
        protein_id: sorted(terms) for protein_id, terms in label_to_propagated.items()
    }

    go_terms = sorted(
        {go_id for terms in label_to_go_terms.values() for go_id in terms}
    )
...
def _extract_go_terms(
    ...
    go_annotation_path = Path(go_annotation_path)

    with go_annotation_path.open("r", encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            direct_terms = {
                term.strip() for term in raw_terms.split(",") if term.strip()
            }
