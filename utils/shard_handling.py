...
class ESMGraphHomologyShardDataset(Dataset):
    def __init__(
        ...
        if len(self.seq_graph_ds) != len(self.homology_ds):
            raise ValueError(
                "Dataset length mismatch between seq+graph and homology datasets."
            )
...
class ESMGraphShardDataset(Dataset):
    ...
    def __init__(
        ...
        if not self.graph_shard_dir.exists():
            raise FileNotFoundError(
                f"Graph shard dir not found: {self.graph_shard_dir}"
            )
    ...
    def __getitem__(self, idx: int):
        ...
        if local_idx >= len(reps):
            raise IndexError(
                f"local_idx={local_idx} out of bounds for ESM shard {shard_id}"
            )
        if local_idx >= len(graphs):
            raise IndexError(
                f"local_idx={local_idx} out of bounds for graph shard {shard_id}"
            )
