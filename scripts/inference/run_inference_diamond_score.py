#!/usr/bin/env python3
"""Evaluate the DiamondScore baseline with CAFA metrics."""
...
from models.diamond_score_baseline import evaluate_diamond_score
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate DiamondScore baseline (bitscore-normalized) with CAFA metrics."
    )
    ...
    parser.add_argument("--subject_go_index_path", type=Path, default=None)
    parser.add_argument(
        "--diamond_hits_path",
        type=Path,
    ...
    parser.add_argument("--evalue_max", type=float, default=1e-5)
    parser.add_argument("--min_query_coverage", type=float, default=0.30)
    parser.add_argument(
        "--outdir",
        type=Path,
        default=PROJECT_ROOT / "runs/inference/diamond_score",
    )
...
def main() -> None:
    ...
    if args.mode != "evaluate":
        raise ValueError("run_inference_diamond_score.py supports only --mode evaluate")
    ...
    subject_go_index_path = (
        PROJECT_ROOT / "diamond_db" / args.go_aspect / "subject_go_index.json"
        if args.subject_go_index_path is None
        else args.subject_go_index_path
    )
    ...
    search_config = DiamondSearchConfig(
        evalue_max=float(args.evalue_max),
        min_query_coverage=float(args.min_query_coverage),
        top_k=int(args.top_k),
    )

    metrics, _curve, _y_prob = evaluate_diamond_score(
        y_true=y_true,
        ...
        diamond_hits_path=args.diamond_hits_path,
