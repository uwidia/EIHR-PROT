...
from utils.pool_embeddings import FusionMLP, NeuralLogitHead
...
from utils.pool_embeddings import ESMSequenceBranch, GATBranch
...
def multimodal_collate_fn_generator(label_to_indices, num_go_terms):
    def multimodal_collate_fn(batch):
        ...
        for i, (item, rep, graph, label) in enumerate(zip(batch, reps, graphs, labels)):
            ...
            mean_conf = torch.tensor(
                float(graph["mean_confidence"]), dtype=torch.float32
            )
...
def fit_reliability_aware_model(
    model,
    train_loader,
    val_loader,
    optimizer,
...
def run_one_batch_smoke_test(
    ...
    assert (
        homology_priors.shape == targets.shape
